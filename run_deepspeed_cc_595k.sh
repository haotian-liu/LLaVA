...
deepspeed --include=localhost:$GPUS --master_port $PORT \
    ...
    --mm_use_im_start_end \
    --bf16 True \
    --output_dir ./checkpoints/deepspeed_llava-13b-$WEIGHT_VERSION-pretrain \
    --num_train_epochs 1 \
...
python scripts/extract_mm_projector.py \
  --model_name_or_path ./checkpoints/deepspeed_llava-13b-$WEIGHT_VERSION-pretrain \
  --output ./checkpoints/mm_projector/deepspeed_llava-13b-$WEIGHT_VERSION-pretrain.bin
...
deepspeed --include=localhost:$GPUS --master_port $PORT \
    ...
    --vision_tower openai/clip-vit-large-patch14 \
    --pretrain_mm_mlp_adapter ./checkpoints/mm_projector/deepspeed_llava-13b-$WEIGHT_VERSION-pretrain.bin \
    --mm_vision_select_layer -2 \
    --mm_use_im_start_end True \
    --bf16 True \
    --output_dir ./checkpoints/deepspeed_llava-13b-$WEIGHT_VERSION-finetune \
    --num_train_epochs 3 \
...
deepspeed --include=localhost:$GPUS --master_port $PORT \
    ...
    --mm_use_im_start_end True \
    --bf16 True \
    --output_dir ./checkpoints/deepspeed_llava-13b-$WEIGHT_VERSION-finetune_lora \
    --num_train_epochs 3 \
    --per_device_train_batch_size 8 \
    --per_device_eval_batch_size 4 \
    --gradient_accumulation_steps 1 \
